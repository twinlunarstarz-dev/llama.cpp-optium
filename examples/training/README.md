...
## Isolated teacher dataset generation
...
`tools.json` must be an OpenAI-compatible JSON array of tool declarations. Tool outputs are fixture data supplied in `tool_results`: this utility deliberately does not execute tools, access search engines, or invent tool results. It supports one tool-call round and a final teacher response. A task with an actual tool call but no matching fixture fails rather than fabricating a result.

The output records have a `category` and an OpenAI-compatible `messages` array. This output is **not yet directly consumable as chat-supervised examples** by `llama-finetune`, which currently tokenizes a text input without chat role/loss masking. Do not pass the JSONL to its `--file` option and assume tool-call supervision works. Chat-template-aware tokenization and assistant target masks are still required.
