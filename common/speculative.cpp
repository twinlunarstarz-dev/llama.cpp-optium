...
common_params common_base_params_to_speculative(const common_params & params) {
    ...
    if (has_draft) {
        // Default to the target/global device list when no draft devices were explicitly pinned.
        ...
        // A single-device drafter should use its native layer path instead of inheriting
        // the target's multi-device tensor split and Meta wrapper.
        const size_t n_draft_devices = std::count_if(
            params_spec.devices.begin(), params_spec.devices.end(),
            [](ggml_backend_dev_t dev) { return dev != nullptr; }
        );
        if (n_draft_devices == 1) {
            result.split_mode = LLAMA_SPLIT_MODE_LAYER;
            for (float & split : result.tensor_split) {
                split = 0.0f;
            }
        }
