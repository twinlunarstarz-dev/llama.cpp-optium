...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--lmcache"}, "HOST:PORT",
        "LMCache bundled-daemon endpoint for cross-process prompt state reuse (default: disabled)",
        [](common_params & params, const std::string & value) {
            params.lmcache_endpoint = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_LMCACHE"));
    add_opt(common_arg(
        {"--metrics"},
