...
| `--lmcache HOST:PORT` | LMCache bundled-daemon endpoint for optional cross-process prompt state reuse (default: disabled). This uses the daemon's generic CPU-memory object protocol, not LMCache's vLLM/SGLang connectors or tiered storage.<br/>(env: LLAMA_ARG_LMCACHE) |
