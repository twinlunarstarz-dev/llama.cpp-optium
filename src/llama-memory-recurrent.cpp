...
void llama_memory_recurrent::clear(bool data) {
    ...
    std::fill(seq_tail.begin(), seq_tail.end(), -1);
}
...
bool llama_memory_recurrent::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    ...
    if (0 <= seq_id) {
        int32_t & tail_id = seq_tail[seq_id];
        if (tail_id >= 0) {
    ...
    for (uint32_t i = 0; i < size; ++i) {
        if (cells[i].pos >= p0 && cells[i].pos < p1) {
            ...
            if (cells[i].is_empty()) {
                ...
                if (seq_id >= 0 && seq_tail[seq_id] == (int32_t) i) {
                    seq_tail[seq_id] = -1;
                }
            }
        }
...
void llama_memory_recurrent::seq_cp(llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) {
    ...
    if ((uint32_t) seq_id_dst < n_seq_max && (uint32_t) seq_id_src < n_seq_max) {
        const int32_t tail_src = seq_tail[seq_id_src];
        int32_t & tail_dst = seq_tail[seq_id_dst];
        if (tail_dst >= 0) {
...
void llama_memory_recurrent::seq_add(llama_seq_id seq_id, llama_pos p0, llama_pos p1, llama_pos shift) {
    ...
    if (0 <= seq_id && seq_id < (int64_t) n_seq_max) {
        const int32_t tail_id = seq_tail[seq_id];
        if (tail_id >= 0) {
            auto & cell = cells[tail_id];
...
bool llama_memory_recurrent::prepare(const std::vector<llama_ubatch> & ubatches) {
    ...
    seq_tail = std::move(org_seq_tail);
    used = org_used;
    head = org_head;
...
bool llama_memory_recurrent::find_slot(const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        for (uint32_t j = 0; j < n_seq_id; ++j) {
            ...
            if (j > 0) {
                int32_t & seq_tail_id = seq_tail[seq_id];
                if (seq_tail_id >= 0) {
                    auto & cell = cells[seq_tail_id];
...
#ifndef NDEBUG
    {
        ...
        for (uint32_t i = 0; i < size; ++i) {
            ...
            for (llama_seq_id seq_id : cell.seq_id) {
                if (seq_id < 0 || (uint32_t) seq_id >= n_seq_max) {
                    LLAMA_LOG_ERROR("%s: invalid seq_id %d in cell %d\n", __func__, seq_id, i);
                    continue;
                }
        ...
        for (uint32_t i = 0; i < n_seq_max; ++i) {
            if (tails_verif[i] != seq_tail[i]) {
                LLAMA_LOG_ERROR("%s: wrong tail for seq_id %d, (%d instead of %d)\n", __func__, i, seq_tail[i], tails_verif[i]);
            }
        }
    }
#endif
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        int32_t & seq_meta_tail = seq_tail[seq_id];
        bool has_cell = false;
        if (seq_meta_tail >= 0) {
            auto & cell = cells[seq_meta_tail];
        ...
        if (!has_cell) {
            ...
            if (seq_meta_tail >= 0) {
                auto & orig_cell = cells[seq_meta_tail];
        ...
        if (min > seq_meta_tail) { min = seq_meta_tail; }
        if (max < seq_meta_tail) { max = seq_meta_tail; }
    }
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        const int32_t src_id = seq_tail[ubatch.seq_id[i][0]];
        if (dst_id != src_id) {
